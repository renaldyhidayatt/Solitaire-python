...
class Text:
    ...
    def display(self, game_display):
        ...
        if self.centered:
            text_rect.center = (
                self.display_width // 2 + self.x_offset,
...
class Radio:
    def __init__(
        self, display_dimensions, offsets, centered=True, checked=False, enabled=True
    ):
        self.display_width, self.display_height = display_dimensions
        self.x_offset, self.y_offset = offsets
        self.centered = centered
...
class Checkbox:
    ...
    def check_for_mouse_over(self, mouse_pos):
        mouse_x, mouse_y = mouse_pos
        if (
            self.x < mouse_x < self.x + self.size
            and self.y < mouse_y < self.y + self.size
        ):
            return True
        else:
            return False
    ...
    def display(self, game_display):
        ...
        pygame.draw.rect(
            game_display, self.color, [self.x, self.y, self.size, self.size]
        )

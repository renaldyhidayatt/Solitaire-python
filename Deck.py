...
class Deck:
        ...
        if self.selection == False:
            ...
            if self.selected_pile != None:
                (
                    ...
                    self.selected_cards,
                    deselect_pile,
                ) = self.selected_pile.selected(mouse_position, self.piles)
                if deselect_pile:
                    self.deselect()
                else:
                    if len(self.selected_cards) != 0:
                        self.selection_rect = self.selected_pile.selection_rect(
            ...
            if self.selected_pile != None and pile_to_transfer_to != None:
                valid_move = self.selected_pile.transfer_cards(
    ...
    def display(self, game_display):
        for pile in self.piles:
            if (
                pile.pile_type == "foundation"
                or pile.pile_type == "deck"
                and len(pile.cards) == 0
            ):
                pygame.draw.rect(
                    game_display,
                    self.empty_color,
            ...
            for card in pile.cards:
                if (
                    self.selection
                    and self.selection_rect != None
                    and card == self.selected_cards[0]
                ):
                    pygame.draw.rect(

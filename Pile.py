...
class Pile:
    def __init__(self, cards, x, y, card_size, pile_type="tableau") -> None:
    ...
    def valid_transfer(self, pile_to_transfer_to, selected_cards, ranks):
        ...
        if (
            pile_to_transfer_to.pile_type == "stock"
            or pile_to_transfer_to.pile_type == "waste"
        ):
            valid = False
    ...
    def selection_rect(self, card):
        ...
        rect_h = (
            (self.card_spacing * (distance_from_top - 2))
            + self.card_height
            + (padding * 2)
        )
    ...
    def check_if_clicked(self, mouse_position):
        ...
        if (
            self.x < mouse_x < self.x + self.card_width
